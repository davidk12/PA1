...
public class PA1Agent implements Agent
{
	...
	public void init(Collection<String> percepts) {
		...
		*/
		Pattern perceptNamePattern = Pattern.compile("\\(\\s*([^\\s]+).*"); // The valid base pattern
		for (String percept:percepts) {
			Matcher perceptNameMatcher = perceptNamePattern.matcher(percept); // Match together the percept and the valid strings
			if (perceptNameMatcher.matches()) { // If there is a match
